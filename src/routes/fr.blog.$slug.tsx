...
export const Route = createFileRoute("/fr/blog/$slug")({
  loader: async ({ params }) => {
    const bundle = await getArticleBundle({ data: { lang: "fr", slug: params.slug } });
  ...
  head: ({ loaderData }) => {
    ...
    const url = `https://acciowork.pro/fr/blog/${a.slug}`;
...
function ArticleFr() {
  ...
  return (
    <I18nProvider>
      <BlogArticle lang="fr" article={article} related={related} />
    </I18nProvider>
  );
}
