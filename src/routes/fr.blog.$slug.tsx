...
export const Route = createFileRoute("/fr/blog/$slug")({
  loader: async ({ params }) => {
    const article = await getArticle({ data: { lang: "fr", slug: params.slug } });
    ...
    return {
      meta: a
        ? [
            ...
            { property: "og:url", content: `https://acciowork.pro/fr/blog/${a.slug}` },
            ...(a.cover_url ? [{ property: "og:image", content: a.cover_url }] : []),
          ]
        : [{ title: "Blog Accio Work" }],
      links: a ? [{ rel: "canonical", href: `https://acciowork.pro/fr/blog/${a.slug}` }] : [],
      scripts: a
        ? [
            {
              ...
              children: JSON.stringify({
                ...
                mainEntityOfPage: { "@type": "WebPage", "@id": `https://acciowork.pro/fr/blog/${a.slug}` },
              }),
            },
          ]
        : [],
  ...
  notFoundComponent: () => <div className="p-10 text-center">Article introuvable</div>,
});
...
function ArticleFr() {
  ...
  return (
    <I18nProvider>
      <BlogArticle lang="fr" article={article} />
    </I18nProvider>
  );
}