...
import { scChrome } from "@/lib/scorecard-data";
import { validateScSearch, scOg, scOgImageUrl } from "@/lib/scorecard-og";

const LANG = "de" as const;
const LANGS = ["en","ru","de","it","es","zh","pt","hi","fr"] as const;

export const Route = createFileRoute("/de/supplier-scorecard")({
  validateSearch: validateScSearch,
  loaderDeps: ({ search }) => search,
  loader: ({ deps }) => ({ og: scOg(deps, LANG), ogImage: scOgImageUrl(deps, LANG) }),
  head: ({ loaderData }) => {
    const c = scChrome[LANG];
    const url = "https://acciowork.pro/de/supplier-scorecard";
    ...
    return {
      ...
      scripts: [
        { type: "application/ld+json", children: JSON.stringify({ "@context":"https://schema.org","@type":"BreadcrumbList", itemListElement:[{"@type":"ListItem",position:1,name:"Accio Work",item:"https://acciowork.pro/de/"},{"@type":"ListItem",position:2,name:c.h1,item:url}] }) },
      ],
    };
  },
  component: () => <SupplierScorecard lang={LANG} />,
});
