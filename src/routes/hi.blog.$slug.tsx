...
export const Route = createFileRoute("/hi/blog/$slug")({
  loader: async ({ params }) => {
    const bundle = await getArticleBundle({ data: { lang: "hi", slug: params.slug } });
  ...
  head: ({ loaderData }) => {
    ...
    if (!a) {
      return {
        meta: [{ title: "Accio Work ब्लॉग" }, { name: "robots", content: "noindex, follow" }],
        links: [],
        scripts: [],
      };
    }
    const url = `https://acciowork.pro/hi/blog/${a.slug}`;
    ...
    return {
      meta: [
        { title: a.title.length <= 45 ? `${a.title} | Accio Work ब्लॉग` : a.title },
      ...
      scripts: [
        {
          ...
          children: JSON.stringify({
            ...
            isPartOf: { "@type": "Blog", name: "Accio Work ब्लॉग", url: `https://acciowork.pro/hi/blog` },
        ...
        },
        {
          ...
          children: JSON.stringify({
            ...
            "@type": "BreadcrumbList",
            itemListElement: [
              { "@type": "ListItem", position: 1, name: "Accio Work", item: `https://acciowork.pro/hi` || "https://acciowork.pro" },
              { "@type": "ListItem", position: 2, name: "Accio Work ब्लॉग", item: `https://acciowork.pro/hi/blog` },
...
function ArticleHi() {
  ...
  return (
    <I18nProvider>
      <BlogArticle lang="hi" article={article} related={related} />
    </I18nProvider>
  );
}
