      ...
      <main className="mx-auto max-w-3xl px-5 py-10 sm:py-14">
        {!done ? (
          <>
            ...
            <p className="mt-3 max-w-2xl text-lg leading-relaxed text-white/75">{c.intro}</p>
            ...
            <div className="mt-8 mb-6">
              <div className="mb-2 flex justify-between text-xs font-medium text-white/58">
                  ...
                  return (
                    <button key={i} onClick={() => pick(i)}
                      className={`flex w-full items-center justify-between gap-3 rounded-2xl border px-5 py-4 text-left text-[15px] font-medium transition ${
                        sel ? "border-[#34d399] bg-white/[0.05] text-[#e8eef9]" : "border-white/12 bg-white/[0.06] text-white/80 hover:border-[#34d399]/40"
                      }`}>
            ...
            <div className="mt-6 flex items-center justify-between">
              <button onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}
                className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium text-white/68 disabled:opacity-40">
          ...
          <>
            ...
            <div className={`rounded-3xl border p-7 sm:p-9 ${tier === "weak" ? "border-white/12 bg-white" : "border-[#34d399]/30 bg-white/[0.05]"}`}>
              <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">{result.title[lang]}</h1>
              <p className="mt-4 text-[17px] leading-relaxed text-white/80">{result.body[lang]}</p>

              {result.cta ? (
                <div className="mt-7">
                  <p className="mb-3 text-sm text-white/68">{c.tryLine}</p>
                  <a href={REFERRAL_URL} target="_blank" rel="noopener noreferrer nofollow"
                  ...
                  <p className="mt-2 text-xs text-white/58">{c.ctaNote}</p>
                </div>
              ) : (
                <div className="mt-7">
                  <p className="text-sm text-white/68">{c.weakNote}</p>
                </div>
              )}
            </div>

            <div className="mt-6 flex items-center gap-4">
              <button onClick={reset} className="inline-flex items-center gap-1.5 text-sm font-medium text-white/68 hover:text-white/90">
                <RotateCcw className="h-4 w-4" /> {c.retake}
              </button>
              <button onClick={onShare} className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/[0.06] px-4 py-2 text-sm font-semibold text-[#e8eef9] transition hover:border-[#34d399]/50">
