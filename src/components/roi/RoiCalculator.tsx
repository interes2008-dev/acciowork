...
const IMG_ALT: Record<RoiLang, string> = {
  ...
  ar: "ساعة بسهم للأعلى وعملات، وقت ومال موفّران",
};
...
export function RoiCalculator({ lang }: { lang: RoiLang }) {
  ...
  useEffect(() => {
    ...
    if (o !== null) {
      ...
      if (Number.isFinite(n)) setOffload(Math.max(30, n));
    }
    const r = q.get("r");
    if (r !== null) {
  ...
  async function onShare() {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: roiChrome[lang].h1, url: shareUrl });
        return;
      }
    } catch {
      /* cancelled */
  ...
  return (
    <Shell lang={lang}>
      <main className="mx-auto max-w-4xl px-5 py-10 sm:py-14">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-border bg-card p-6 sm:p-7">
            ...
            <div className="mt-6 border-t border-border pt-5">
              ...
              />
              <p className="mt-1.5 text-xs text-foreground/52">{c.offloadHint}</p>
            </div>
            <div className="mt-5 border-t border-border pt-5">
              <label className="flex cursor-pointer items-center gap-2 text-sm font-medium text-foreground/75">
                <input
                  type="checkbox"
                  checked={showMoney}
                  onChange={(e) => setShowMoney(e.target.checked)}
                  className="h-4 w-4 rounded accent-[#34d399]"
                  style={{ accentColor: "#34d399" }}
                />
                {c.hourlyToggle}
              </label>
              {showMoney && (
                <div className="mt-3 flex flex-wrap items-center gap-3">
                  <span className="text-sm text-foreground/68">{c.hourlyLabel}</span>
                  <select
                    value={cur}
                  ...
                  <input
                    ...
                    className="w-24 rounded-lg border border-border bg-secondary px-3 py-1.5 text-sm"
                  />
                  <span className="text-sm text-foreground/52">{c.perHour}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-gradient-to-br from-[#0a1120] to-[#0f2e26] p-6 text-white sm:p-7">
            ...
            {showMoney && rate > 0 && (
              <div className="mt-6 flex gap-6 border-t border-white/15 pt-5">
                <div>
        ...
        <p className="mt-8 max-w-3xl text-xs leading-relaxed text-foreground/52">{c.disclaimer}</p>
      </main>
    </Shell>
  );
}
